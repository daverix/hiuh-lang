...
class _BaseTokenizerTests:
    ...
    def test_variables_math_section(self):
        ...
        expected = [Token(TOKEN_SET, 'sätt', 1, 1), Token(TOKEN_IDENTIFIER, 'c', 1, 6), Token(TOKEN_TO, 'till', 1, 8), Token(TOKEN_IDENTIFIER, 'b', 1, 13), Token(TOKEN_OP_MUL, 'gånger', 1, 15), Token(TOKEN_IDENTIFIER, 'b', 1, 22), Token(TOKEN_OP_ADD, 'plus', 1, 24), Token(TOKEN_IDENTIFIER, 'a', 1, 29)]
        self.assertEqual(self.tokenize(source), expected)

    def test_variables_float_section(self):
        source = "sätt y till 3,4"
        expected = [Token(TOKEN_SET, 'sätt', 1, 1), Token(TOKEN_IDENTIFIER, 'y', 1, 6), Token(TOKEN_TO, 'till', 1, 8), Token(TOKEN_LITERAL_FLOAT, '3,4', 1, 13)]
        self.assertEqual(self.tokenize(source), expected)

    def test_variables_list_section(self):
        source = "sätt min lista till lista med 1, 2, 3"
        expected = [Token(TOKEN_SET, 'sätt', 1, 1), Token(TOKEN_IDENTIFIER, 'min', 1, 6), Token(TOKEN_IDENTIFIER, 'lista', 1, 10), Token(TOKEN_TO, 'till', 1, 16), Token(TOKEN_IDENTIFIER, 'lista', 1, 21), Token(TOKEN_WITH, 'med', 1, 27), Token(TOKEN_LITERAL_INT, '1', 1, 31), Token(TOKEN_COMMA, ',', 1, 32), Token(TOKEN_LITERAL_INT, '2', 1, 34), Token(TOKEN_COMMA, ',', 1, 35), Token(TOKEN_LITERAL_INT, '3', 1, 37)]
        self.assertEqual(self.tokenize(source), expected)
    ...
    def test_stdin_section(self):
        ...
        expected = [Token(TOKEN_SET, 'sätt', 1, 1), Token(TOKEN_IDENTIFIER, 't', 1, 6), Token(TOKEN_TO, 'till', 1, 8), Token(TOKEN_IDENTIFIER, 'nästa', 1, 13), Token(TOKEN_IDENTIFIER, 'rad', 1, 19), Token(TOKEN_FROM, 'från', 1, 23), Token(TOKEN_IDENTIFIER, 'inmatning', 1, 28)]
        self.assertEqual(self.tokenize(source), expected)
    ...
    def test_tokenize_list_operations(self):
        ...
        source = 'sätt element 0 i minlista till röd\nskriv element 0 från minlista'
        tokens = self.tokenize(source)
        expected_types = [TOKEN_SET, TOKEN_IDENTIFIER, TOKEN_LITERAL_INT, TOKEN_IDENTIFIER, TOKEN_IDENTIFIER, TOKEN_TO, TOKEN_IDENTIFIER, TOKEN_NEWLINE, TOKEN_PRINT, TOKEN_IDENTIFIER, TOKEN_LITERAL_INT, TOKEN_FROM, TOKEN_IDENTIFIER]
    ...
    def test_bootstrapping_sanity_tokens(self):
        ...
        expected = [Token(TOKEN_OPEN, 'öppna', 1, 1), Token(TOKEN_IDENTIFIER, 'källkod_test.hiuh', 1, 7), Token(TOKEN_FOR, 'för', 1, 25), Token(TOKEN_IDENTIFIER, 'läsning', 1, 29), Token(TOKEN_AS, 'som', 1, 37), Token(TOKEN_IDENTIFIER, 'fil', 1, 41), Token(TOKEN_NEWLINE, '\n', 1, 44), Token(TOKEN_SET, 'sätt', 2, 1), Token(TOKEN_IDENTIFIER, 'rad_nummer', 2, 6), Token(TOKEN_TO, 'till', 2, 17), Token(TOKEN_LITERAL_INT, '1', 2, 22), Token(TOKEN_NEWLINE, '\n', 2, 23), Token(TOKEN_WHILE, 'medan', 4, 1), Token(TOKEN_IDENTIFIER, 'inte', 4, 7), Token(TOKEN_IDENTIFIER, 'i', 4, 12), Token(TOKEN_IDENTIFIER, 'slutet', 4, 14), Token(TOKEN_FROM, 'från', 4, 21), Token(TOKEN_IDENTIFIER, 'fil', 4, 26), Token(TOKEN_NEWLINE, '\n', 4, 29), Token(TOKEN_INDENT, '    ', 5, 1), Token(TOKEN_SET, 'sätt', 5, 5), Token(TOKEN_IDENTIFIER, 'rad', 5, 10), Token(TOKEN_TO, 'till', 5, 14), Token(TOKEN_IDENTIFIER, 'nästa', 5, 19), Token(TOKEN_IDENTIFIER, 'rad', 5, 25), Token(TOKEN_FROM, 'från', 5, 29), Token(TOKEN_IDENTIFIER, 'fil', 5, 34), Token(TOKEN_NEWLINE, '\n', 5, 37), Token(TOKEN_SET, 'sätt', 8, 5), Token(TOKEN_IDENTIFIER, 'första_tecken', 8, 10), Token(TOKEN_TO, 'till', 8, 24), Token(TOKEN_IDENTIFIER, 'element', 8, 29), Token(TOKEN_LITERAL_INT, '0', 8, 37), Token(TOKEN_FROM, 'från', 8, 39), Token(TOKEN_IDENTIFIER, 'rad', 8, 44), Token(TOKEN_NEWLINE, '\n', 8, 47), Token(TOKEN_PRINT, 'skriv', 10, 5), Token(TOKEN_IDENTIFIER, 'rad_nummer', 10, 11), Token(TOKEN_OP_ADD, 'plus', 10, 22), Token(TOKEN_IDENTIFIER, '.', 10, 27), Token(TOKEN_OP_ADD, 'plus', 10, 29), Token(TOKEN_IDENTIFIER, 'första_tecken', 10, 34), Token(TOKEN_OP_ADD, 'plus', 10, 48), Token(TOKEN_IDENTIFIER, 'mellanrum', 10, 53), Token(TOKEN_NEWLINE, '\n', 10, 62), Token(TOKEN_SET, 'sätt', 11, 5), Token(TOKEN_IDENTIFIER, 'rad_nummer', 11, 10), Token(TOKEN_TO, 'till', 11, 21), Token(TOKEN_IDENTIFIER, 'rad_nummer', 11, 26), Token(TOKEN_OP_ADD, 'plus', 11, 37), Token(TOKEN_LITERAL_INT, '1', 11, 42), Token(TOKEN_NEWLINE, '\n', 11, 43), Token(TOKEN_DEDENT, '', 13, 1), Token(TOKEN_CLOSE, 'stäng', 13, 1), Token(TOKEN_IDENTIFIER, 'fil', 13, 7)]
        self.assertEqual(self.tokenize(source), expected)
    ...
    def test_infix_grej_tokens(self):
        ...
        expected = [Token(TOKEN_SET, 'sätt', 1, 1), Token(TOKEN_IDENTIFIER, 'innehåller', 1, 6), Token(TOKEN_TO, 'till', 1, 17), Token(TOKEN_IDENTIFIER, 'infixgrej', 1, 22), Token(TOKEN_WITH, 'med', 1, 32), Token(TOKEN_IDENTIFIER, 'lista', 1, 36), Token(TOKEN_AS, 'som', 1, 42), Token(TOKEN_IDENTIFIER, 'lista', 1, 46), Token(TOKEN_OF, 'av', 1, 52), Token(TOKEN_IDENTIFIER, 'heltal', 1, 55), Token(TOKEN_COMMA, ',', 1, 61), Token(TOKEN_IDENTIFIER, 'värde', 1, 63), Token(TOKEN_AS, 'som', 1, 69), Token(TOKEN_IDENTIFIER, 'heltal', 1, 73), Token(TOKEN_RETURNS, 'ger', 1, 80), Token(TOKEN_IDENTIFIER, 'boolesk', 1, 84)]
        self.assertEqual(self.tokenize(source), expected)
    ...
    def test_bryt_fortsätt_in_while_body(self):
        ...
        expected = [Token(TOKEN_WHILE, 'medan', 1, 1), Token(TOKEN_IDENTIFIER, 'x', 1, 7), Token(TOKEN_OP_IS, 'är', 1, 9), Token(TOKEN_LESS, 'mindre', 1, 12), Token(TOKEN_THAN, 'än', 1, 19), Token(TOKEN_LITERAL_INT, '10', 1, 22), Token(TOKEN_NEWLINE, '\n', 1, 24), Token(TOKEN_INDENT, '    ', 2, 1), Token(TOKEN_IF, 'om', 2, 5), Token(TOKEN_IDENTIFIER, 'x', 2, 8), Token(TOKEN_OP_IS, 'är', 2, 10), Token(TOKEN_EQUAL, 'lika', 2, 13), Token(TOKEN_WITH, 'med', 2, 18), Token(TOKEN_LITERAL_INT, '5', 2, 22), Token(TOKEN_NEWLINE, '\n', 2, 23), Token(TOKEN_INDENT, '        ', 3, 1), Token(TOKEN_BREAK, 'bryt', 3, 9), Token(TOKEN_NEWLINE, '\n', 3, 13), Token(TOKEN_DEDENT, '', 4, 1), Token(TOKEN_ELSE, 'annars', 4, 5), Token(TOKEN_NEWLINE, '\n', 4, 11), Token(TOKEN_INDENT, '        ', 5, 1), Token(TOKEN_CONTINUE, 'fortsätt', 5, 9), Token(TOKEN_DEDENT, '', 6, 1), Token(TOKEN_DEDENT, '', 6, 1)]
        self.assertEqual(self.tokenize(source), expected)
...
class TestHiuhTokenizer(_BaseTokenizerTests, unittest.TestCase):
    ...
    def _tokenize_via_hiuh(self, source):
        ...
        hiuh_source = f'använd tokeniserare\n\nsätt rader till lista med {line_strings}\n\nge tokenisera med rader\n'
        ...
        try:
            ...
            if isinstance(result, list):
                return [(t.get('tokentyp', -1), str(t.get('värde', '')), t.get('rad', 0), t.get('kolumn', 0)) for t in result]
        return []
